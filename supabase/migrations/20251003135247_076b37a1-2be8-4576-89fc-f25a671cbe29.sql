...
-- 9. Empêcher les utilisateurs de modifier le champ role dans profiles
-- Créer une fonction trigger pour bloquer la modification du rôle
...
AS $$
BEGIN
  -- Si c'est un admin qui fait la modification, autoriser
  IF public.has_role(auth.uid(), 'admin') THEN
    RETURN NEW;
  END IF;
  
  -- Sinon, bloquer la modification du rôle
...
DROP TRIGGER IF EXISTS prevent_role_modification_trigger ON public.profiles;
...
-- 12. Fonction pour créer automatiquement un rôle lors de la création d'un profil
