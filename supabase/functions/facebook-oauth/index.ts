...
Deno.serve(async (req) => {
  ...
  try {
    const { code, userId, state, redirectUri, accessToken, isMobileSDK } = await req.json();

    // Pour le SDK mobile natif, on reçoit directement un accessToken
    if (isMobileSDK && accessToken) {
      console.log('📱 SDK Mobile natif détecté - utilisation du token fourni');
      ...
      const debugTokenUrl = `https://graph.facebook.com/v21.0/debug_token?input_token=${accessToken}&access_token=${appAccessToken}`;
      
      console.log('🔍 Validation du token mobile...');
      const debugResponse = await fetch(debugTokenUrl);
      const debugData = await debugResponse.json();
      
      if (debugData.data?.is_valid !== true) {
        console.error('❌ Token invalide:', debugData);
        throw new Error('Invalid access token from mobile SDK');
      }
      
      console.log('✅ Token mobile validé');

      // Continuer avec le flux normal de récupération des pages
      ...
      if (!pagesData.data || pagesData.data.length === 0) {
        throw new Error('Aucune page Facebook trouvée. Assurez-vous d\'avoir des pages associées à votre compte.');
      }
      ...
      for (const page of pagesData.data) {
        const { data, error } = await supabaseClient
          ...
          .upsert({
            ...
            expires_at: null, // Le SDK natif gère l'expiration
          }, {
      ...
      console.log(`✅ ${connections.length} page(s) connectée(s)`);
