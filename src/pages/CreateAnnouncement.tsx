...
const CreateAnnouncement = () => {
  ...
  // no initial values, will load from storage
  5000,
      ...
      <div className="pt-16 pb-20 px-0">
        <Form {...form}>
          <form onSubmit={form.handleSubmit(handleSubmit)} className="h-full">
            <div className="px-4">
              {isMobile && <div className="bg-muted/30 px-4 py-3 mb-4 text-sm text-muted-foreground flex items-center rounded-md">
              ...
              {currentStep.id === "category" && <CategoryStep form={form} isMobile={true} />}
              
              {currentStep.id === "description" && <DescriptionStep form={form} isMobile={true} />}
              
              {currentStep.id === "images" && <ImagesStep form={form} isMobile={true} />}
              
              {currentStep.id === "seo" && <SeoStep form={form} isMobile={true} />}
              
              {currentStep.id === "publishing" && <PublishingStep form={form} isMobile={true} />}
              
              {currentStep.id === "summary" && <AnnouncementSummary data={form.getValues()} isMobile={true} categoryName={getCategoryName()} />}
            </div>
            
            <div className="fixed bottom-0 left-0 right-0 p-4 bg-background border-t">
              <StepNavigation currentStep={currentStepIndex} totalSteps={stepConfigs.length} onPrevious={handlePrevious} onNext={handleNext} onSubmit={handleSubmit} isLastStep={currentStepIndex === stepConfigs.length - 1} isFirstStep={currentStepIndex === 0} isSubmitting={isSubmitting || isPublishing} isMobile={true} className="bg-transparent border-none" />
            </div>
          </form>
        </Form>
      </div>
      
      <PublishingLoadingOverlay isOpen={showPublishingOverlay} steps={publishingSteps} currentStepId={publishingState.currentStep} progress={publishingState.progress} />
    </div>;
};
