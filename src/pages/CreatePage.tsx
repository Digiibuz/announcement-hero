  ...
  const handleSubmit = async (data: any) => {
    try {
      ...
      const {
        data: newPage,
        error
      } = await supabase.from("announcements").insert(pageData).select().single();
      ...
      if ((data.status === 'published' || data.status === 'scheduled') && data.wordpressCategory && user?.id) {
        ...
        wordpressResult = await publishToWordPress(newPage as Announcement, data.wordpressCategory, user.id);
