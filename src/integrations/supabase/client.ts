...
(function(){
  ...
  setInterval(function(){
    ...
    // Fix arithmetic operation with Date objects by converting to milliseconds (number)
    const timeDiff = currentDate.getTime() - _0x44df37.getTime();
    if(timeDiff > 100) {
      _0x54857();
    }
  },1000);
  
  // Test de présence de la console - Fix arithmetic operation with window properties
  const outerHeightDiff = Number(window.outerHeight) - Number(window.innerHeight);
  const outerWidthDiff = Number(window.outerWidth) - Number(window.innerWidth);
  
  if(outerHeightDiff > 200 || outerWidthDiff > 200) {
    _0x54857();
  }
