  ...
  return {
    ...
    toast: (content: React.ReactNode | ExternalToast, options?: ExternalToast) => {
      if (typeof content === 'string' || React.isValidElement(content)) {
        return baseToast({ ...options, description: content });
      }
