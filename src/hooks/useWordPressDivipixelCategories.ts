...
export const useWordPressDivipixelCategories = () => {
  ...
  useEffect(() => {
    console.log("useWordPressDivipixelCategories effect running, user:", user?.id, "wordpressConfigId:", user?.wordpressConfigId);
    if (user?.wordpressConfigId) {
      fetchCategories();
    }
