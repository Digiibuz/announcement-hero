...
export const useDiviPixelCategories = () => {
  ...
  const fetchCategories = useCallback(async () => {
    ...
    try {
      ...
      console.log("WordPress config found for DiviPixel:", {
      ...
      if (wpConfig.app_username && wpConfig.app_password) {
        console.log("Using Application Password authentication for DiviPixel");
        const basicAuth = btoa(`${wpConfig.app_username}:${wpConfig.app_password}`);
        headers['Authorization'] = `Basic ${basicAuth}`;
      } else if (wpConfig.rest_api_key) {
        console.log("Using REST API Key authentication for DiviPixel");
        headers['Authorization'] = `Bearer ${wpConfig.rest_api_key}`;
      } else {
        console.log("No authentication credentials provided for DiviPixel");
      }
      ...
      try {
        ...
        if (!response.ok) {
          ...
          console.error("DiviPixel API error:", response.status, errorText);
        ...
        if (fetchError.name === 'AbortError') {
          throw new Error("Le délai d'attente a expiré lors de la récupération des catégories DiviPixel");
        }
        throw fetchError;
      }
    } catch (err: any) {
  ...
  useEffect(() => {
    console.log("useDiviPixelCategories effect running, user:", user?.id, "wordpressConfigId:", user?.wordpressConfigId);
    if (user?.wordpressConfigId) {
      fetchCategories();
    }
