...
import ImageUploader from "@/components/announcements/ImageUploader";
import DescriptionField from "@/components/announcements/DescriptionField";
import PublishingOptions from "@/components/announcements/PublishingOptions";
  ...
  useEffect(() => {
    if (initialValues) {
      Object.keys(initialValues).forEach((key) => {
        const typedKey = key as keyof PageFormData;
  ...
  const optimizeSeoContent = async (field: 'seoTitle' | 'seoDescription') => {
    try {
      ...
      if (!currentTitle || !currentDescription) {
        toast.warning("Veuillez d'abord saisir le titre et la description de la page");
        return;
      }
            ...
            <div className={`${isMobile ? "px-4" : ""}`}>
              <Card className={getCardStyles(true)}>
                <CardHeader className={`${isMobile ? "px-0 py-3" : "pb-3"}`}>
                  <CardTitle className="text-lg font-medium">Votre page</CardTitle>
                  {!isMobile && <CardDescription className="text-amber-400">
                      Les informations essentielles de votre page
                    </CardDescription>}
                </CardHeader>
                <CardContent className={`space-y-4 ${isMobile ? "px-0 py-3" : ""}`}>
                  ...
                  field
                        ...
                        <FormControl>
                          <Input placeholder="Entrez le titre de la page" className="h-11" {...field} />
                        </FormControl>
                        <FormMessage />
                      </FormItem>} />
            ...
            <div className={`${isMobile ? "px-4" : ""}`}>
              <Card className={getCardStyles(true)}>
                <CardHeader className={`${isMobile ? "px-0 py-3" : "pb-3"}`}>
                  ...
                  {!isMobile && <CardDescription className="text-amber-400">
                      Ajoutez des images à votre page pour attirer l'attention
                    </CardDescription>}
                </CardHeader>
                  ...
                  field
                        ...
                        <FormControl>
                          <div className="flex items-center">
                            <span className="text-sm text-muted-foreground mr-2 hidden sm:inline">yoursite.com/</span>
                            <Input placeholder="slug-de-url" {...field} />
                          </div>
                        </FormControl>
                        <FormDescription>
                          L'URL qui sera utilisée pour accéder à cette page.
                        </FormDescription>
                        <FormMessage />
                      </FormItem>} />
                  ...
                  {!isMobile && <Card className="border border-muted mt-4 bg-muted/10">
                      <CardContent className="p-4">
                        ...
                        <div className="text-blue-600 text-lg font-medium truncate">
                          {form.getValues('seoTitle') || form.getValues('title') || "Titre de votre page"}
                        </div>
                        <div className="text-green-700 text-sm mb-1">
                          yoursite.com/{form.getValues('seoSlug') || "url-de-la-page"}
                        </div>
